            ...
            return <li key={k}>
              <div className="item flex my-5">
                ...
                <div className='flex items-center justify-center font-semibold w-1/3 text-lg'>
                  <CiCircleMinus className='cursor-pointer' onClick={() => removeFromCart(k, 1)} />
                  <span className='mx-2 text-sm'>{cart[k].qty}</span>
                  <CiCirclePlus className='cursor-pointer' onClick={() => addToCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant)} />
                </div>
              </div>
            </li>
          })}
